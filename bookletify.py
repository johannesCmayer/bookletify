import argparse
import os
import pdb
from subprocess import check_output
from shutil import which, copy

SCRIPT_PATH = os.path.dirname(os.path.realpath(__file__))
TITLE_PAGE_PATH_NO_EX = f"{SCRIPT_PATH}/templates/title"
BLANK_PAGE_PATH_NO_EX = f"{SCRIPT_PATH}/templates/blank"

def is_pdf(string):
    if os.path.isfile(string) and os.path.splitext(string)[-1] == ".pdf":
        return string
    else:
        raise Exception(f"{string} is not a pdf file")

def wrap_book(content_pdf_path, title_pdf_path, blank_pdf_path):
    file_path = "wrap.pdf"

    os.system(f"pdfunite {title_pdf_path} {blank_pdf_path} {content_pdf_path} {file_path}")
    num_pages = num_of_pages(file_path)
    trash_paths = []
    while num_pages % 4 != 0:
        dst = f"temp-{num_pages}-{file_path}"
        trash_paths.append(dst)
        os.system(f"pdfunite {file_path} {blank_pdf_path} {dst}")
        num_pages += 1

    copy(f"temp-{num_pages-1}-{file_path}", file_path)
    for p in trash_paths:
        os.remove(p)
    return file_path

def num_of_pages(pdf_file_path):
    num_pages = check_output(f"pdftk {pdf_file_path} dump_data | grep NumberOfPages | awk '{{print $2}}'", shell=True)
    return int(num_pages)

def main():
    for p in ["pdftk", "pdfnup"]:
        assert which(p) is not None, f"Script needs '{p}' executables in path."

    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', dest="input_path", required=True, type=is_pdf, help="Input pdf file.")
    parser.add_argument('-o', '--output', dest="output_path", required=True, help="output pdf file.")
    args = parser.parse_args()

    print("Building templates")
    os.system(f"pdflatex {BLANK_PAGE_PATH_NO_EX}.tex")
    blank_page_pdf = f"{BLANK_PAGE_PATH_NO_EX}.pdf"
    os.system(f"pdflatex {TITLE_PAGE_PATH_NO_EX}.tex")
    title_page_pdf = f"{TITLE_PAGE_PATH_NO_EX}.pdf"

    print("Wraping book")
    wraped_pdf = wrap_book(args.input_path, title_page_pdf, blank_page_pdf)

    print("Bookletify wraped book")
    num_pages = num_of_pages(wraped_pdf)

    print(f"Number of pages in pdf: {num_pages}")

    l = []

    swap = True

    assert num_pages % 4 == 0, "Only can handle pdf with page count divisible by 4"
    assert num_pages % 2 == 0

    for i in range(1, int(num_pages/2) + 1):
        if swap:
            l.append(num_pages + 1 - i)
            l.append(i)
            swap = False
        else:
            l.append(i)
            l.append(num_pages + 1 - i)
            swap = True

    num_str = ""
    for i,e in enumerate(l):
        num_str += str(e) 
        if i < len(l) - 1:
            num_str += " "

    temp_file = f"temp_{args.output_path}"

    os.system(f"pdftk {wraped_pdf} cat {num_str} output {temp_file}")
    os.system(f"pdfnup --nup 2x1 --outfile {args.output_path} {temp_file}")
    os.remove(temp_file)

if __name__ == "__main__":
    main()
